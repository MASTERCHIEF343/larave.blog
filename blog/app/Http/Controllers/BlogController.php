	...
	public function sinin(StoreNewauthorRequest $request){
		...
		if($email){
			...
			if($passwd == $requestpasswd){
				...
				return view('register.login',['err'=>'Your password is wrong !']);
			}
		}else{
			return view('register.login',['err'=>'Your email is not exsits !']);
		}
	}
	//注册页面
	...
	*/
	public function user(Request $request){
		if($request->session()->has('name')){
			...
			return view('user')->with('name',$username);
		}else{
			abort(401);
		}
	}
	...
	*/
	public function sendmail(Request $request){
		...
		if($email = User::where('email','=',$request->email)->first()){
			...
			return view('reset',['err'=>'Your email is not exsits !']);
		}
	}
}
